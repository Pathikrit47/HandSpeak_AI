...
class Application:
    ...
    def __init__(self):
        ...
        self.speak_engine.setProperty("rate", 140)  # clean, clear speed
        ...
        self.speak_engine.setProperty("voice", selected_voice)
        ...
        self.speak.place(x=600, y=button_y)
        self.speak.config(text="Speak", font=("Courier", 20), wraplength=100, command=self.speak_fun)

        self.clear = tk.Button(self.root)
        self.clear.place(x=960, y=button_y)
        self.clear.config(text="Clear", font=("Courier", 20), wraplength=100, command=self.clear_fun)
        
        # next button
        self.next_button = tk.Button(self.root, text="Next", font=("Courier", 16), command=self.next_sentence)
        self.next_button.place(x=1350, y=630)

        # History side panel
        self.history_label = tk.Label(self.root, text="History", font=("Courier", 18, "bold"))
        self.history_label.place(x=1100, y=10)

        self.history_box = tk.Text(self.root, width=28, height=25, font=("Courier", 14))
        self.history_box.place(x=1100, y=50)

        # Scrollbar for history panel
        self.scrollbar = tk.Scrollbar(self.root, command=self.history_box.yview)
        self.scrollbar.place(x=1345, y=50, height=400)

        self.history_box.config(yscrollcommand=self.scrollbar.set)


        # Save as TXT button
        self.save_button = tk.Button(self.root, text="Save History", font=("Courier", 16), command=self.save_history)
        self.save_button.place(x=1080, y=button_y)

        # Space Button
        self.space_button = tk.Button(self.root)
        self.space_button.place(x=840, y=button_y)
        self.space_button.config(text="Space", font=("Courier", 20), wraplength=100, command=self.space_fun)

        # Quit Button
        self.quit_button = tk.Button(self.root)
        self.quit_button.place(x=720, y=button_y)
        self.quit_button.config(text="Quit", font=("Courier", 20), wraplength=100, command=self.destructor)
    ...
    def speak_fun(self):
        text_to_say = self.str.strip()
        if text_to_say:
            subprocess.run(['say', '-v', 'Alex', text_to_say])

            entry = f"{self.history_counter}. {text_to_say}\n"
            self.history_box.insert(tk.END, entry)
            self.history_box.see(tk.END)
            self.history_counter += 1

            # Mark this sentence as already saved to history
    ...
    def save_history(self):
        history_text = self.history_box.get("1.0", tk.END).strip()
        if not history_text:
            print("No history to save.")
            return

        with open("sentence_history.txt", "w") as file:
            file.write(history_text)

        print("History saved to sentence_history.txt")



    def next_sentence(self):
        text_to_save = self.str.strip()

        if text_to_save and not self.already_saved:
            entry = f"{self.history_counter}. {text_to_save}\n"
            self.history_box.insert(tk.END, entry)
            self.history_box.see(tk.END)
            self.history_counter += 1
        ...
        # Reset the saved flag for the next sentence
        self.already_saved = False
